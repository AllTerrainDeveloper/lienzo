...
#
# Ported from the OpenStation plugin's own release.sh, minus the parts this
# repository has no use for: there is no in-tree types package to keep in
# lockstep and no translation catalogue to refresh, so there is no --skip-i18n.
# The branch here is `main`, and the version lives in four places rather than
# five.
#
# Flags:
...
for arg in "$@"; do
	case "$arg" in
		...
		--dry-run-changelog)
			...
			;;
		-h|--help)
			echo "usage: $0 <version> [--skip-changelog] [--dry-run-changelog]  (e.g., 1.0.0 or 1.0.0-rc1)"
			exit 0
			;;
		-*)
			echo "error: unknown flag '$arg'" >&2
			echo "usage: $0 <version> [--skip-changelog] [--dry-run-changelog]" >&2
			exit 1
			;;
		*)
			if [[ -n "$new" ]]; then
...
if [[ -z "$new" ]]; then
	echo "usage: $0 <version> [--skip-changelog] [--dry-run-changelog]  (e.g., 1.0.0 or 1.0.0-rc1)" >&2
	exit 1
fi
...
# `daguerre` while the plugin is `allterrain-photo-editor`, so nothing here
...
# Leftovers from an aborted release attempt are regenerated or re-reviewed on
# every run and belong in the bump commit, so they must not block a re-run: a
# drafted-but-unconfirmed changelog in readme.txt, and the version strings
# themselves. The version files are on this list because `bump-version.sh` runs
# before the changelog gate, so answering `n` there leaves them written but
# uncommitted; without the exemption the re-run that gate promises would abort
# instead of returning to the prompt. `bump-version.sh` rewrites them
# deterministically on every run, so a stale value cannot survive.
#
# Anything else dirty still aborts: the bump uses `git commit -am` and would
# silently sweep it up. That deliberately includes assets/ — the built bundles
# are committed on purpose, and CI rebuilds and diffs them, so an uncommitted
# rebuild has to be dealt with as its own commit rather than ride along here.
dirty=$(git status --porcelain --untracked-files=no | grep -vE '^.{3}(readme\.txt$|package\.json$|package-lock\.json$|allterrain-photo-editor\.php$)' || true)
if [[ -n "$dirty" ]]; then
...
# skip the catch-up. These are the same four the release workflow
# re-checks against the tag before it will publish anything.
# awk (instead of `grep -oP`) for BSD/macOS portability — `-P` is GNU-only.
pkg=$(node -p "require('./package.json').version")
header=$(awk '/^[[:space:]]*\*[[:space:]]*Version:/ { print $3; exit }' allterrain-photo-editor.php)
constant=$(awk -F"'" '/LIENZO_VERSION/ { print $4; exit }' allterrain-photo-editor.php)
...
# normally.
if git diff HEAD --quiet -- package.json package-lock.json allterrain-photo-editor.php; then
	bump_committed=1
else
	bump_committed=0
fi
...
if [[ "$pkg" == "$new" && "$header" == "$new" && "$constant" == "$new" && "$stable" == "$new" && "$bump_committed" == "1" ]]; then
	...
	# and would NOT reach the tag. Force them through main first.
	if ! git diff --quiet -- readme.txt; then
		...
		echo "    git add readme.txt && git commit -m \"Update $new changelog\" && git push origin main" >&2
		exit 1
	fi
else
	...
	#
	# On the very first release there is no previous tag to diff against,
	# so the draft step reports that and steps aside; the hand-written
	# block in readme.txt goes to the gate unchanged.
	...
	if git diff --quiet; then
		echo "Nothing to commit — versions already in sync."
	else
...
for i in $(seq 1 100); do
	run_id=$(gh run list --repo "$repo" --branch main --workflow ci.yml --commit "$sha" --limit 1 --json databaseId -q '.[0].databaseId' 2>/dev/null || true)
	[[ -n "$run_id" ]] && break
